...
final class MediaUploadServer: Sendable {
    ...
    ///   - internalClient: GutenbergKit's own client for the configured site.
    ...
    /// `EditorViewController.warmup()` passes no handlers, so it never starts a server.
    ///
    ...
    private static func countServerStarted(
        ...
        let handler = processor.map { String(describing: type(of: $0)) }
        ...
        Logger.uploadServer.fault(
            ...
            and is its own media handler (\(handler, privacy: .public)) forms a retain cycle \
            ARC cannot break, so the editor's deinit never runs. Call \
